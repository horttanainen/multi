...
const PIANO_MELODY_PHRASE: composition.PhraseGenerator = .{
    .anchor = 10,
    .region_low = 7,
    .region_high = 16,
    .rest_chance = 0.48,
...
const PIANO_HARMONY_PHRASE: composition.PhraseGenerator = .{
    .anchor = 9,
    .region_low = 5,
    .region_high = 13,
    .rest_chance = 0.62,
...
const PIANO_DRONE = dsp.midiToFreq(36);
...
var piano_layer: layers.PianoVoicePairLayer = .{};
var drone_layer: layers.DroneLayer = .{};

pub fn reset() void {
    rng = dsp.Rng.init(77777);
    lfo_reverb = .{ .period_beats = 140, .depth = 0.05 };
    layers.resetPianoVoicePairLayer(&piano_layer, PIANO_MELODY_PHRASE, PIANO_HARMONY_PHRASE);
    layers.resetDroneLayer(&drone_layer, instruments.SineDrone.init(PIANO_DRONE, 120.0, 1.002, 1.0, 0.5, 0.02));
...
pub fn fillBuffer(buf: [*]f32, frames: usize) void {
    ...
    for (0..frames) |i| {
        ...
        if (frame.voice_triggers[MELODY_VOICE]) {
            layers.maybeTriggerPianoMelody(&piano_layer, &rng, &runner.engine.key, .{
                .rest_chance = rest_chance,
                .brightness = brightness,
                .meso = frame.tick.meso,
                .micro = frame.tick.micro,
            });
        }

        if (frame.voice_triggers[HARMONY_VOICE]) {
            layers.maybeTriggerPianoHarmony(&piano_layer, &rng, &runner.engine.key, .{
                .rest_chance = rest_chance,
                .brightness = brightness,
                .meso = frame.tick.meso,
                .micro = 0.0,
        ...
        const piano_out = layers.mixPianoVoicePairLayer(&piano_layer, &rng, brightness, note_vol * runner.layer_levels[MELODY_LAYER], note_vol * runner.layer_levels[HARMONY_LAYER]);
        left += piano_out[0];
        right += piano_out[1];

        const drone_out = layers.mixDroneLayer(&drone_layer, runner.layer_levels[DRONE_LAYER]);
        left += drone_out[0];
        right += drone_out[1];
...
fn advanceChord() void {
    ...
    layers.applyPianoPairChord(&piano_layer, &runner.engine.harmony, runner.engine.key.scale_type);
    layers.applyDroneChord(&drone_layer, runner.engine.key.root, chord, -1);
...
fn applyCueParams() void {
    ...
    layers.applyPianoPhraseCue(&piano_layer, .{
        .melody_phrase = spec.melody_phrase,
        .harmony_phrase = spec.harmony_phrase,
    });
