...
fn parseInstrumentName(name: []const u8) ?InstrumentName {
    ...
    if (std.mem.eql(u8, name, "guitar-faust-body-pluck") or std.mem.eql(u8, name, "guitar_faust_body_pluck")) {
        return .guitar_faust_body_pluck;
    }
    return null;
}
...
fn writeInstrumentFrames(file: std.Io.File, cfg: RenderConfig, frequency_hz: f32, total_frames: u32) !RenderStats {
